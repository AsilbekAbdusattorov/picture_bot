...
// 🌍 CORS muammolarini yechish
app.use(cors({ origin: "https://picture-bot.vercel.app", credentials: true }));

app.use(bodyParser.json({ limit: "10mb" })); // Rasm uchun limit qo‘shildi

app.post("/upload", (req, res) => {
    const { image, userId } = req.body;

    if (!image || !userId) {
        return res.status(400).json({ error: "Image yoki UserID yetishmayapti!" });
    }

    console.log(`Rasm qabul qilindi: ${userId}`);
